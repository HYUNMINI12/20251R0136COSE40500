...
def parse_stata_string(input_string):
    ...
    pattern = r'(reg .*?_cons[^\n]*\n|Source.*?_cons[^\n]*\n)'
    sections = re.findall(pattern, input_string, re.DOTALL)
    regno = 0
    ...
    for section in sections:
        ...
        if match:
            ...
            #independent_variables_match = re.findall(r"[\w.#]+", section, re.MULTILINE)
            #independent_variables_match = re.findall(r'\b(\w+[#\w.]*\w+)\b', section, re.MULTILINE)
...
input = os.path.join(current_dir, "myway1.log")
df = parse_stata_file(input)
